...
const ConfirmationOutpatient = ({ handleConfirm, type }: { handleConfirm: () => void, type: OutpatientType }) => {
  ...
  const buttonAction = [
    { action: () => { setOpenConfirmDialog(false) }, label: "Periksa Kembali", buttonProps: { variant: "outlined", size: "large", color: "secondary", fullWidth: true } },
    { action: () => { handleConfirm() }, label: "Konfirmasi Pendaftaran", buttonProps: { variant: "contained", size: "large", color: "secondary", fullWidth: true } },
  ]
    ...
    <Stack gap={4}>
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <Typography gutterBottom variant="h5" color="secondary.darker">Detail Pasien</Typography>
          <LabelTextContainer listText={detailData} orientation="horizontal" />
        </Grid>

        <Grid item xs={12} md={6}>
          <Typography gutterBottom variant="h5" color="secondary.darker">Detail Kunjungan</Typography>
          <Grid container spacing={2}>
            {
      ...
      <Box sx={{ display: "flex", placeContent: "space-between", gap: 2 }}>
        <Button color="secondary" fullWidth variant="outlined" size="large">Ubah Pendaftaran</Button>
        <Button
          fullWidth
          color="secondary"
          variant="contained"
